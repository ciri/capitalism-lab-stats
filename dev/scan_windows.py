...
def scan_windows(save_paths: list[Path], stride_min: int, stride_max: int, stride_step: int, start_step: int, sample_records: int) -> dict:
	...
	for stride in range(stride_min, stride_max + 1, stride_step):
		max_start = max(0, min_len - stride * sample_records)
		for start in range(0, max_start + 1, start_step):
			window_records = {
	...
	best = sorted(results, key=lambda item: item["avg_cross_save_record_delta"])[:40]
	return {
		"parameters": {
...
def main() -> None:
	...
	report   = scan_windows(saves, args.stride_min, args.stride_max, args.stride_step, args.start_step, args.sample_records)
